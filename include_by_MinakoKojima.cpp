...
struct qry{
    ...
    int l, r, m, id; LL z;
    void in(){
        RD(l, r); l = l * 2; r = r * 2; --l; ++r;
        m = (l + r) / 2; id = Qn++; z = -((r-l)/2+1);
...
VI query_event[N], leave_event[N];
...
int main(){
    ...
    REP(i, Qn) query_event[Q[i].l].PB(Q[i].id);
    FOR(i, 1, n) leave_event[i-r[i]].push_back(i);
 
    DWN(i, n, 1){
        C0.add(i, i); C1.add(i, 1);
        for(auto &t: leave_event[i]){
            C0.add(t, r[t]-t), C1.add(t, -1);
        }
        for(auto &t: query_event[i]){
            Q[t].z += C0.sum(Q[t].m)-C1.sum(Q[t].m)*(i-1);
        }
    }
    ...
    REP(i, Qn) query_event[Q[i].r].PB(Q[i].id);
    FOR(i, 1, n) leave_event[i+r[i]].push_back(i);
 
    FOR(i, 1, n){
        C0.add(n-i, i); C1.add(n-i, 1);
        for(auto &t: leave_event[i]){
            C0.add(n-t, -r[t]-t); C1.add(n-t, -1);
        }
        for(auto &t: query_event[i]){
            Q[t].z += C1.sum(n-(Q[t].m+1))*(i+1)-C0.sum(n-(Q[t].m+1));
        }
    }
